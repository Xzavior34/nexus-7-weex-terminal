...
export function LiveMarketCard({
  ...
  useEffect(() => {
    ...
    prevLivePriceRef.current = liveCurrentPrice;
  ...
  }, [basePrice, volatility, currentPrice, isLive]);
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-border/50 relative z-10">
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs text-muted-foreground">
            {isLive ? `${data.length} pts (last ~${Math.round((data.length * 5000) / 60000)}m)` : "Vol: $1.2B"}
          </span>
        </div>
